...
SCRIPT = "WizardDiary.sol:WizardDiary"
# WizardDiary's constructor takes the secret diary entry, which players recover
# by reading storage slot 1
CONSTRUCTOR_ARGS = "0x000000000000000000000000000000000000000000000000000000deadbeef37"
FLAG = "pecan{pr1v473_isnt_pr1v473?}"
...
def deploy_contract():
    ...
    output = subprocess.run(f"forge create {SCRIPT} --private-key {addresses[1]} --broadcast --constructor-args {CONSTRUCTOR_ARGS}", shell=True, capture_output=True, text=True)
    if output.stderr != "":
